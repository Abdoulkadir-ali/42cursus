...
/**
 * @brief THE UNIFIED geometry.hIT CONTRACT
 */
...
bool						bvh_occluded(const t_bvh *bvh, const t_ray *ray,
								double max_dist);
bool						process_leaf_occluded(const t_bvh *bvh,
								int node_idx, const t_ray *ray, double max_t);
void						push_occ_children(t_occ *v, int node_idx);
void						push_children(t_bvh_stack *s, int node_idx);
void						build_emissive_cache(t_scene *scene);
t_bvh_tmp_node				*build_tmp_tree(t_scene *scene, size_t total,
								size_t *k, t_build_item **final_items);
...
/* --- SHADING --- */
t_vec3						calc_light(t_shading *sha, const t_rt_engine *rt,
								t_light light);
void						add_emissive_lighting(t_shading *sha,
								const t_rt_engine *rt, t_vec3 *total);
void						apply_emissive_hit(t_shading *sha, t_vec3 *total,
								t_material *m, t_vec3 lrad);
void						emissive_primary(t_shading *sha,
								const t_rt_engine *rt, t_vec3 *tot,
								t_emissive_ref r);
void						emissive_complex(t_shading *sha,
								const t_rt_engine *rt, t_vec3 *tot,
								t_emissive_ref r);
