...
static bool	is_in_shadow(t_scene *scene, const t_bvh *bvh, t_vec3 point, t_vec3 light_dir, double light_dist)
{
	...
	// Use Early Exit BVH occlusion check
	if (bvh_occluded(bvh, &shadow_ray, light_dist))
		return (true);
...
t_vec3	compute_color(t_hit *hit, t_scene *scene, const t_bvh *bvh, const t_ray *ray, int depth)
{
	...
	for (i = 0; i < scene->light_count; i++)
	{
		...
		t_vec3	light_dir = vec3_sub(light.transform.pos, hit->point);
		...
		{
			// light.transform.forward is direction OF the light
			...
			double angle = vec3_dot(l_to_p, light.transform.forward); // light.transform.forward should be normalized
