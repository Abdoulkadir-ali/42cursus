...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/16 18:00:00 by abdoali           #+#    #+#             */
/*   Updated: 2026/02/16 18:00:00 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static t_vec3 closest_point_on_triangle(t_vec3 p, t_vec3 v0, t_vec3 v1, t_vec3 v2)
{
    ...
    return vec3_add(v0, vec3_add(vec3_scale(edge0, s), vec3_scale(edge1, t)));
}

/*
 * Checks if a sphere collides with a single triangle.
 */
static bool test_sphere_triangle(const struct s_sphere *s, t_vec3 v0, t_vec3 v1, t_vec3 v2,
    t_vec3 *out_normal, double *out_penetration, double *min_dist_sq)
{
    ...
    if (dist_sq < radius * radius && dist_sq < *min_dist_sq)
    {
        *min_dist_sq = dist_sq;
        ...
        if (out_normal) *out_normal = vec3_scale(diff, 1.0 / (dist + 1e-6)); // Avoid div by zero
 ...
 */
static bool detect_sphere_capsule_collision(const struct s_sphere *s, const t_collider *c,
    ...
    t_vec3 *out_normal, double *out_penetration)
{
	...
	if (get_physics_state()->mesh_simplify_collision && m->collider.type == COLLIDER_CAPSULE)
	{
		return (detect_sphere_capsule_collision(s, &m->collider, out_normal, out_penetration));
	}

    if (!m || !m->bvh_nodes) return false;

    /* Transform sphere into mesh local space? 
       Actually, our BVH is likely in World Space if we rebuilt it, 
       BUT miniRT meshes usually have a local BVH and a world transform.
       
       Let's assume the BVH is in local space AABB, but the vertices are ???
       Wait, `mesh_build_bvh` builds on `mesh->vertices`.
       If `mesh->vertices` are local, we must transform the sphere to local space.
       
       Checking `mesh_apply_transform`: it modifies vertices directly. 
       So vertices are largely in World Space after parsing?
       Original code: `mesh_apply_transform` bakes transform.
       If vertices are baked, everything is World Space. Excellent.
    */

    /* Broad phase: Sphere vs Mesh AABB */
    t_aabb s_aabb = sphere_aabb((t_sphere*)s);
    if (!aabb_overlap(&m->bbox, &s_aabb)) return false;
    ...
    while (top > 0)
    {
        ...
        if (!aabb_overlap(&node->bbox, &s_aabb)) continue;
    ...
    if (hit)
    {
        ...
        if (out_penetration) *out_penetration = best_pen;
    }
    return hit;
}
