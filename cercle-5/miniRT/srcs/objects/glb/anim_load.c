		...
		t_json_value *sampler_json)
{
	...
	/* Assuming Vec3 or Vec4. Stride might vary. We store as floats.
		...
		We do via 'type' string in accessor,
			but glb_parse_accessor doesn't return that string easily.
	...
	*/
	...
	/* If we allocated 4 but only read 3,
		the 4th is garbage. That's fine if we only read 3 later. */
	/* Interpolation */
