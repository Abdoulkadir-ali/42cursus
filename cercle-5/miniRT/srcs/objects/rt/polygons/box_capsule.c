...
t_parse_obj	parse_box(t_parser *p)
{
	...
	res.data.box.phys.mass = 1.0;
	res.data.box.phys.elasticity = 0.5;
	res.data.box.phys.friction = 0.5;
	res.type = TYPE_BOX;
	return (res);
}
...
t_parse_obj	parse_capsule(t_parser *p)
{
	...
	res.data.capsule.phys.mass = 1.0;
	res.data.capsule.phys.elasticity = 0.5;
	res.data.capsule.phys.friction = 0.5;
