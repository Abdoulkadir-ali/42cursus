...
t_parse_obj	parse_tri_shape(t_parser *p)
{
	...
	res.data.tri_shape.xform.pos = vec3_scale(
			...
			1.0 / 3.0);
	res.data.tri_shape.phys.mass = 1.0;
	res.data.tri_shape.phys.elasticity = 0.5;
	res.data.tri_shape.phys.friction = 0.5;
	res.type = TYPE_TRI;
	return (res);
}
...
t_parse_obj	parse_rect(t_parser *p)
{
	...
	res.data.rect.phys.mass = 1.0;
	res.data.rect.phys.elasticity = 0.5;
	res.data.rect.phys.friction = 0.5;
	res.type = TYPE_RECT;
	return (res);
}
...
t_parse_obj	parse_pyramid(t_parser *p)
{
	...
	res.data.pyramid.phys.mass = 1.0;
	res.data.pyramid.phys.elasticity = 0.5;
	res.data.pyramid.phys.friction = 0.5;
