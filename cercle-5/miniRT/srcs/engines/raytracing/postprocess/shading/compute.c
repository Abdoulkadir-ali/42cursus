...
static void	setup_shading(t_shading *sha, t_hit *hit, const t_rt_engine *rt)
{
	sha->hit = hit;
	sha->rt = rt;
		...
		double *kr, double next_w)
{
	...
	ior = 1.0 + (sha->mat->refract_index / 100.0) * 1.5; /* Constants should be defined safely */
	...
	if (vec3_dot(ref_dir, sha->hit->normal) > 0)
		ref_ray.origin = vec3_add(sha->hit->point,
				vec3_scale(sha->hit->normal, sha->rt->settings.shadow_bias));
	else
		ref_ray.origin = vec3_sub(sha->hit->point,
				vec3_scale(sha->hit->normal, sha->rt->settings.shadow_bias));
...
static t_vec3	compute_reflection(t_shading *sha, const t_ray *ray, double nw)
{
	...
	ray_init(&ref_ray, vec3_add(sha->hit->point, 
		vec3_scale(n, sha->rt->settings.shadow_bias)), ref_dir);
	ref_ray.depth = ray->depth + 1;
	ref_ray.weight = nw;
	return (trace_ray((t_rt_engine *)sha->rt, &ref_ray));
}
...
t_vec3	compute_color(t_hit *hit, t_rt_engine *rt, const t_ray *r)
{
	...
	total = pixel_color(sha.albedo, rt->scene->ambient.rgb, rt->scene->ambient.brightness);
	i = -1;
	if (r->depth < 2)
		while (++i < (int)rt->scene->light_count)
			total = vec3_add(total, calc_light(&sha, rt, rt->scene->lights[i]));
	if (r->depth == 0)
