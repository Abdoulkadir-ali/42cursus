...
bool	realloc_physics_soa(t_physics_soa *p, size_t new_cap)
{
	...
	nvx = realloc(p->vx, new_cap * 4 * 3); /* linear v 3 components */
	nax = realloc(p->ang_vx, new_cap * 4 * 3); /* angular v 3 components */
	...
	am = realloc(p->amin_x, new_cap * 4 * 3);
	ax = realloc(p->amax_x, new_cap * 4 * 3);
	...
	if (!ni || !nvx || !nax || !nm || !ne || !ni_inv || !am || !ax || !ns || !nc || !nf || !npc)
		return (false);
	p->prim_idx = ni;
	p->vx = nvx; p->vy = nvx + new_cap; p->vz = nvx + 2 * new_cap;
	p->ang_vx = nax; p->ang_vy = nax + new_cap; p->ang_vz = nax + 2 * new_cap;
	...
	p->amin_x = am; p->amin_y = am + new_cap; p->amin_z = am + 2 * new_cap;
	p->amax_x = ax; p->amax_y = ax + new_cap; p->amax_z = ax + 2 * new_cap;
...
bool	scene_add_physics(t_scene *s, int prim_idx)
{
	t_physics_soa	*p = &s->physics;
	int				idx;
	...
	s->primitives.phys_idx[prim_idx] = idx;
	p->vx[idx] = 0; p->vy[idx] = 0; p->vz[idx] = 0;
	p->ang_vx[idx] = 0; p->ang_vy[idx] = 0; p->ang_vz[idx] = 0;
