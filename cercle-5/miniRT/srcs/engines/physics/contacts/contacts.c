...
static int	sphere_vs_capsule(t_sphere *sp, t_capsule *cap, t_contact *c)
{
	...
	d = vec3_sub(sp->phys.pos, closest);
	...
	c->rb = vec3_sub(closest, cap->phys.pos);
...
static int	box_vs_plane(t_box *bx, t_plane *pl, t_contact *c, int max_c)
{
	...
	while (i < 8 && count < max_c)
	{
		...
		if (dist < 0.0)
		{
			...
			c[count].ra = vec3_sub(corner, bx->phys.pos);
...
static int	rect_vs_plane(t_rect *rc, t_plane *pl, t_contact *c, int max_c)
{
	...
	while (i < 4 && count < max_c)
	{
		...
		if (dist < 0.0)
		{
			...
			c[count].ra = vec3_sub(rc->v[i], rc->phys.pos);
			c[count].rb = vec3(0, 0, 0);
				...
				int max_c)
{
	...
	while (i < 3 && count < max_c)
	{
		...
		if (dist < 0.0)
		{
			...
			c[count].ra = vec3_sub(tr->v[i], tr->phys.pos);
			c[count].rb = vec3(0, 0, 0);
				...
				int max_c)
{
	...
	while (i < 5 && count < max_c)
	{
		...
		if (dist < 0.0)
		{
			...
			c[count].ra = vec3_sub(verts[i], py->phys.pos);
			c[count].rb = vec3(0, 0, 0);
		...
		t_bvh_ref ref, t_contact *c)
{
	...
	if (ref.type == TYPE_SPHERE && ref.index > idx)
	{
		...
		d = vec3_sub(sphere->phys.pos, other->phys.pos);
		dist_sq = vec3_mag_sq(d);
...
static int	sphere_vs_plane_analytic(t_sphere *sp, t_plane *pl, t_contact *c)
{
	t_vec3	n;
	t_vec3	to_center;
	double	dist;
	...
	if (vec3_mag_sq(pl->transform.up) < 1e-6)
		...
		n = vec3_norm(pl->transform.up);
	to_center = vec3_sub(sp->phys.pos, pl->transform.pos);
	dist = vec3_dot(to_center, n);
	r = sqrt(sp->radius_sq);
	if (dist >= r)
		return (0);
		...
		int max)
{
	...
	sa = (t_gjk_shape){sphere, gjk_support_sphere, sphere->phys.pos};
	count = traverse_bvh_contacts(s, idx, sphere, saabb, c, count, max);
	p = 0;
	while (p < s->plane_count && count < max)
	{
		count += sphere_vs_plane_analytic(sphere, &s->planes[p], &c[count]);
		p++;
	}
	p = 0;
