...
/*   Updated: 2026/03/28 18:13:26 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
bool	collision_gjk_epa(t_gjk_shape *a, t_gjk_shape *b, t_simplex *s, t_epa_res *res)
{
	t_epa_poly	poly;
	int			cf;
	t_vec3		p;
	...
	collision_init_polytope(&poly, s);
	iter = -1;
	while (++iter < EPA_MAX_ITER)
	{
		cf = collision_closest_face(&poly);
		...
		if (vec3_dot(poly.faces[cf].normal, p) - poly.faces[cf].dist < EPA_TOL)
		{
			...
			collision_get_contact_points(&poly, &poly.faces[cf], &res->contact_a, &res->contact_b);
			return (true);
		}
		expand(&poly, p, pa, pb);
	}
