...
/*   Updated: 2026/03/26 13:34:00 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int count, int max)
{
	...
	while (p < s->box_count && count < max)
		if (aabb_overlap(baabb, box_aabb(&s->boxes[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &bx->phys, &s->boxes[p].phys,
	...
	while (++p < s->capsule_count && count < max)
		if (aabb_overlap(baabb, capsule_aabb(&s->capsules[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &bx->phys, &s->capsules[p].phys,
	...
	while (++p < s->cylinder_count && count < max)
		if (aabb_overlap(baabb, cylinder_aabb(&s->cylinders[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &bx->phys,
					&s->cylinders[p].phys, &bx->transform,
	...
	while (++p < s->rect_count && count < max)
		if (aabb_overlap(baabb, rect_aabb(&s->rects[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &bx->phys, &s->rects[p].phys,
					&bx->transform, &s->rects[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->tri_count && count < max)
		if (aabb_overlap(baabb, tri_aabb(&s->tris[p])))
		{
			sb = (t_gjk_shape){&s->tris[p], gjk_support_tri,
				s->tris[p].phys.center};
			count += gjk_make_contact(&sa, &sb, &bx->phys, &s->tris[p].phys,
					&bx->transform, &s->tris[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->pyramid_count && count < max)
		if (aabb_overlap(baabb, pyramid_aabb(&s->pyramids[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &bx->phys, &s->pyramids[p].phys,
