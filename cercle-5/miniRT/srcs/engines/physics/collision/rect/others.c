		...
		int count, int max)
{
	...
	while (p < s->rect_count && count < max)
		if (aabb_overlap(raabb, rect_aabb(&s->rects[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &rc->phys, &s->rects[p].phys,
					&rc->transform, &s->rects[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->tri_count && count < max)
		if (aabb_overlap(raabb, tri_aabb(&s->tris[p])))
		{
			sb = (t_gjk_shape){&s->tris[p], gjk_support_tri,
				s->tris[p].phys.center};
			count += gjk_make_contact(&sa, &sb, &rc->phys, &s->tris[p].phys,
					&rc->transform, &s->tris[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->pyramid_count && count < max)
		if (aabb_overlap(raabb, pyramid_aabb(&s->pyramids[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &rc->phys, &s->pyramids[p].phys,
