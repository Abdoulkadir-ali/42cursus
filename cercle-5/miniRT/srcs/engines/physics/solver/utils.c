...
t_vec3	point_vel(t_physics *p, int body_idx, t_vec3 r)
{
	...
	if (body_idx < 0 || !p || !p->soa)
		return (vec3(0, 0, 0));
	s = p->soa;
	return (vec3_add(vec3(s->hot.vx[body_idx], s->hot.vy[body_idx],
				s->hot.vz[body_idx]),
			vec3_cross(vec3(s->hot.ang_vx[body_idx], s->hot.ang_vy[body_idx],
					s->hot.ang_vz[body_idx]), r)));
}
		...
		double inv_m, double sign)
{
	...
	s->hot.ang_vx[body_idx] += dw.x;
	s->hot.ang_vy[body_idx] += dw.y;
	s->hot.ang_vz[body_idx] += dw.z;
}
...
double	ang_term(t_physics *p, int body_idx, t_vec3 r, t_vec3 dir, double inv_m)
{
	...
	return (rxd.x * rxd.x * s->inv_ix[body_idx]
