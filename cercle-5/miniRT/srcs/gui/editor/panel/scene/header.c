...
static void	draw_scene_header_info(t_gui *gui, t_vec2s d)
{
	...
	mlx_string_put_c(gui->win.mlx, gui->win.win, ui_sx(8, d), ui_sy(36, d),
		COL_UI_LIGHT_BLUE, buf);
	snprintf(buf, sizeof(buf), "ROT  %.0f  %.0f",
		c->transform.rotation.pitch * 57.29, c->transform.rotation.yaw * 57.29);
	mlx_string_put_c(gui->win.mlx, gui->win.win, ui_sx(8, d), ui_sy(52, d),
		COL_UI_LIGHT_BLUE, buf);
	mlx_string_put_c(gui->win.mlx, gui->win.win, ui_sx(8, d), ui_sy(68, d),
		COL_UI_MUTED, "WASD Move  SPC/SHF Up/Dn");
	mlx_string_put_c(gui->win.mlx, gui->win.win, ui_sx(8, d), ui_sy(82, d),
...
void	draw_scene_header(t_gui *gui)
{
	...
	mlx_string_put_c(gui->win.mlx, gui->win.win, ui_sx(8, d), ui_sy(16, d),
		COL_ACCENT, "miniRT");
