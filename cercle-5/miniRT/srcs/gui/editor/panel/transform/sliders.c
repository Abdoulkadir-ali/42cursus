...
/*   Updated: 2026/04/06 10:42:01 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	build_pos_sliders(t_gui *gui, t_vec3 *v, t_vec2i *p)
{
	...
	sl = (t_islider){"X", SL_POS_MIN, SL_POS_MAX, &v->x,
		transform_selection_sync};
	draw_insp_row(gui, (t_vec2i){p->x, p->y}, sl);
	p->y += ui_sy(INSP_ROW_STEP, d);
	sl = (t_islider){"Y", SL_POS_MIN, SL_POS_MAX, &v->y,
		transform_selection_sync};
	draw_insp_row(gui, (t_vec2i){p->x, p->y}, sl);
	p->y += ui_sy(INSP_ROW_STEP, d);
	sl = (t_islider){"Z", SL_POS_MIN, SL_POS_MAX, &v->z,
		transform_selection_sync};
	draw_insp_row(gui, (t_vec2i){p->x, p->y}, sl);
	p->y += ui_sy(INSP_ROW_STEP, d);
}
...
static void	build_rot_sliders(t_gui *gui, t_rotator *r, t_vec2i *p)
{
	...
	p->y += ui_sy(INSP_HDR_STEP, d);
	sl = (t_islider){"Pitch", -180.0, 180.0, &r->pitch,
		transform_selection_sync};
	draw_insp_row(gui, (t_vec2i){p->x, p->y}, sl);
	p->y += ui_sy(INSP_ROW_STEP, d);
