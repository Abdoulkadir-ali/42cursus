...
static void	draw_inspector_header(t_gui *gui, t_vec2i pos)
{
	...
	snprintf(buf, sizeof(buf), "%s  #%zu",
		type_name_str(gui->selection.type), gui->selection.index.i);
	mlx_string_put_c(gui->win.mlx, gui->win.win, pos.x + ui_sx(8, d),
		ui_sy(50, d), COL_ACCENT, buf);
}
...
void	draw_inspector_bg(t_gui *gui)
{
	...
	panel = (t_panel){.pos = vec2i(gui->win.disp_size.x - (int)s.x, 0),
		.size = s, .bg = COL_BG, .brd = COL_BORDER};
