...
void	draw_popup_shape(t_gui *gui)
{
	...
	draw_modal_bg(gui);
	gui_draw_string(gui, "Add Object", gui->crud->box.pos.x + POPUP_PAD, gui->crud->box.pos.y + POPUP_PAD, COL_ACCENT);
	i = 0;
	while (i < 11)
	{
		bw = COL_POPUP_BTN;
		if (i == 10)
			bw = 0x1E2A1E;
		draw_popup_btn(gui, vec2i(gui->crud->box.pos.x + POPUP_PAD + (i % 3)
			* (((POPUP_W - POPUP_PAD * 4) / 3) + POPUP_PAD), gui->crud->box.pos.y
			+ 36 + POPUP_PAD + (i / 3) * (POPUP_ITEM_H + 8)),
			vec2i(((POPUP_W - POPUP_PAD * 4) / 3), POPUP_ITEM_H), labs[i], bw);
		i++;
	}
	draw_popup_btn(gui, vec2i(gui->crud->box.pos.x + POPUP_W - 90 - POPUP_PAD,
		gui->crud->box.pos.y + 36 + POPUP_PAD + 4 * (POPUP_ITEM_H + 8) + 4),
...
bool	click_popup_shape(t_gui *gui, t_vec2i mouse)
{
	...
	while (++i < 11)
	{
		if (phit(mouse, (t_gui_box){vec2i(gui->crud->box.pos.x + POPUP_PAD + (i % 3)
			* (bw + POPUP_PAD), gui->crud->box.pos.y + 36 + POPUP_PAD + (i / 3)
	...
	if (phit(mouse, (t_gui_box){vec2i(gui->crud->box.pos.x + POPUP_W - 90 - POPUP_PAD,
		gui->crud->box.pos.y + 36 + POPUP_PAD + 4 * (POPUP_ITEM_H + 8) + 4),
		vec2i(90, 26)}))
		gui->crud->popup = POPUP_NONE;
	return (true);
