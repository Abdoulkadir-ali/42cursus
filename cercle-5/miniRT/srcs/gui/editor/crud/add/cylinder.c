...
void	editor_add_cylinder(t_gui *gui)
{
	...
	params.mat_id = scene_add_material_from_color(gui->scene,
			vec3(0.9, 0.7, 0.4));
	DBG_INFO_MSG(DBG_CH_EDITOR, "editor_add_cylinder: pos=(%.2f,%.2f,%.2f)\n",
		(double)params.pos.x, (double)params.pos.y, (double)params.pos.z);
	pthread_rwlock_wrlock(&gui->scene_lock);
	scene_add_primitive(gui->scene, params, PRIM_CYLINDER);
	DBG_INFO_MSG(DBG_CH_EDITOR, "editor_add_cylinder: prim count now=%zu\n",
		gui->scene->primitives.count);
	select_object(gui, TYPE_CYLINDER, gui->scene->primitives.count - 1);
	rebuild_bvh(gui);
