...
bool	click_popup_mesh_fmt(t_gui *gui, t_vec2i mouse)
{
	...
	if (phit(mouse, vec2i(o.x + ui_sx(POPUP_PAD, d), o.y + ui_sy(36 + POPUP_PAD, d)),
			vec2i(bw, ui_sy(POPUP_ITEM_H, d))))
		gui->crud.popup = POPUP_MESH_PATH;
	if (phit(mouse, vec2i(o.x + ui_sx(POPUP_PAD * 2, d) + bw, o.y + ui_sy(36 + POPUP_PAD, d)),
			vec2i(bw, ui_sy(POPUP_ITEM_H, d))))
		gui->crud.popup = POPUP_MESH_PATH;
	return (true);
}
