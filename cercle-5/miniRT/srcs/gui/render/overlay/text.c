...
void	draw_ui_object(t_gui *gui)
{
	...
	mlx_string_put_c(gui->win.mlx, gui->win.win, 350, 38, 0xE0A820, buf);
	snprintf(buf, sizeof(buf), "Material: %zu", mesh->mat_id);
	mlx_string_put_c(gui->win.mlx, gui->win.win, 350, 58, 0xD0D0D8, buf);
	snprintf(buf, sizeof(buf), "Vertices: %zu", mesh->vertex_count);
	mlx_string_put_c(gui->win.mlx, gui->win.win, 350, 78, 0xD0D0D8, buf);
	snprintf(buf, sizeof(buf), "Transform: Pos(%.2f,%.2f,%.2f)",
		mesh->transform.pos.x, mesh->transform.pos.y, mesh->transform.pos.z);
	mlx_string_put_c(gui->win.mlx, gui->win.win, 350, 98, 0xD0D0D8, buf);
}
