...
void	draw_ui_panels(t_gui *gui)
{
	...
	panel = (t_panel){15, gui->win.height - 65, 450, 50, 0x050505, 0x444444};
	draw_panel(gui, panel);
	panel = (t_panel){gui->win.width - 110, 15, 95, 35, 0x050505, 0x00FF00};
	draw_panel(gui, panel);
}
...
static void	draw_ui_text_footer(t_gui *gui, t_camera_controller *ctrl, int wht)
{
	...
	mlx_string_put(gui->win.mlx, gui->win.win, 30, gui->win.height - 52, wht,
		buf);
	snprintf(buf, sizeof(buf), "ROT: %.1f %.1f", ctrl->transform.rotation.pitch
		* 57.29, ctrl->transform.rotation.yaw * 57.29);
	mlx_string_put(gui->win.mlx, gui->win.win, 30, gui->win.height - 35, wht,
		buf);
}
...
void	draw_ui_text(t_gui *gui, t_camera_controller *ctrl)
{
	...
	mlx_string_put(gui->win.mlx, gui->win.win, gui->win.width - 100, 37,
		0x00FF00, buf);
}
