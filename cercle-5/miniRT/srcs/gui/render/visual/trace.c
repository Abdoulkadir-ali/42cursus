...
static void fill_block(t_render *render)
{
	...
	d.y = 0;
	while ((size_t)d.y < render->step && (size_t)(render->pos.y + d.y) < (size_t)render->gui->win.size.y)
	{
		dst = (uint32_t *)(render->pixel_addr + (d.y * render->gui->win.line_len));
		d.x = 0;
		while ((size_t)d.x < render->step && (size_t)(render->pos.x + d.x) < (size_t)render->gui->win.size.x)
		{
