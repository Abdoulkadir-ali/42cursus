...
/*   Updated: 2026/04/24 20:24:17 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	blend_pixel(struct s_upscale *ctx, int px, float wf[4])
{
	...
	else
	{
		...
		b.x = (ctx->cache[0].x * wf[0] + ctx->cache[1].x * wf[1]
				+ ctx->cache[2].x * wf[2] + ctx->cache[3].x * wf[3]) * rcp_w;
		b.y = (ctx->cache[0].y * wf[0] + ctx->cache[1].y * wf[1]
				+ ctx->cache[2].y * wf[2] + ctx->cache[3].y * wf[3]) * rcp_w;
		b.z = (ctx->cache[0].z * wf[0] + ctx->cache[1].z * wf[1]
				+ ctx->cache[2].z * wf[2] + ctx->cache[3].z * wf[3]) * rcp_w;
		b.w = 0.0f;
		ctx->out[0] = rt_pack_color(b);
	}
}
...
static void	apply_depth_thresh(struct s_upscale *u, int src_x, float wf[4])
{
	...
	if (d00 > 1e-4f)
	{
		thresh_abs = DEPTH_THRESH * d00;
		if (fabsf(d0[1] - d00) > thresh_abs)
			wf[1] = 0.0f;
		if (fabsf(d1[0] - d00) > thresh_abs)
			wf[2] = 0.0f;
		if (fabsf(d1[1] - d00) > thresh_abs)
			wf[3] = 0.0f;
	}
}
__attribute__((optimize("O3")))static void	do_upscale_loop(t_gui *gui, struct s_upscale *u,
				float omy, float fy)
{
	int		x;
	int		src_x;
	...
	x = -1;
	factor = (double)gui->win.size.x / (double)gui->win.disp_size.x;
	if (u->depth[0])
	{
		const float	ffactor = (float)factor;
		while (++x < (int)gui->win.disp_size.x)
		{
			const float	fx = (float)x * ffactor;
			src_x = (int)fx;
			f = fx - (float)src_x;
