...
{
	b->nv = ft_calloc(b->tc * 3, sizeof(t_vec3));
	b->ni = ft_calloc(b->tc * 3, sizeof(int));
	if (b->n)
		b->nn = ft_calloc(b->tc * 3, sizeof(t_vec3));
	if (b->u)
		b->nu = ft_calloc(b->tc * 3, sizeof(t_vec2));
	b->v = malloc(sizeof(int) * b->raw_c);
	if (!b->nv || !b->ni || (b->n && !b->nn) || (b->u && !b->nu) || !b->v)
		return (false);
	return (true);
}
