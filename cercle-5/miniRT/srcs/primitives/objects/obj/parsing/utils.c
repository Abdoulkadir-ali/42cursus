...
void	obj_ensure_out_capacity(t_obj *obj)
{
	if (obj->out_v_count < obj->out_v_cap)
		return ;
	if (!dynarray_ensure((void **)&obj->out_v, obj->out_v_count,
			&obj->out_v_cap, sizeof(t_vec3)))
		return ;
	if (!dynarray_ensure((void **)&obj->out_vt, obj->out_v_count,
			&obj->out_v_cap, sizeof(t_vec2)))
		return ;
	if (!dynarray_ensure((void **)&obj->out_vn, obj->out_v_count,
			&obj->out_v_cap, sizeof(t_vec3)))
		return ;
}
...
void	obj_add_vert(t_obj *obj, int vi, int vti, int vni)
{
	...
	vi = obj_fix_index(vi, obj->v_count);
	vti = obj_fix_index(vti, obj->vt_count);
	vni = obj_fix_index(vni, obj->vn_count);
