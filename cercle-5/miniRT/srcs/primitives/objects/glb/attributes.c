...
/*   Updated: 2026/03/26 19:33:29 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_attribute_vec3(t_json_value *json, char *bin, t_vec3 *dst,
		int id)
{
	t_accessor acc;
	t_buffer_view bv;
	...
	if (!tmp)
		return ;
	glb_extract_data((t_extract_ctx){bin, &acc, &bv, tmp, sizeof(float) * 3,
...
static void	fill_attribute_vec2(t_json_value *json, char *bin, t_vec2 *dst,
		int id)
{
	t_accessor acc;
	t_buffer_view bv;
	...
	/* If it's unsigned byte (5121) or unsigned short (5123),
	...
	if (acc.component_type != 5126)
	{
		...
		/* TODO: Implement conversion if we see this warning. For now,
			assume float but warn. */
	}

	tmp = malloc(sizeof(float) * 2 * acc.count);
	if (!tmp)
		return ;
	glb_extract_data((t_extract_ctx){bin, &acc, &bv, tmp, sizeof(float) * 2,
	...
	while (++i < (int)acc.count)
	{
		/* GLTF uses top-left origin,
			we might need to flip V depending on our texture mapper */
...
static void	fill_joints(t_json_value *json, char *bin, t_bone_weight *dst,
		int id)
{
	t_accessor acc;
	t_buffer_view bv;
	...
	glb_extract_data((t_extract_ctx){bin, &acc, &bv, tmp, comp_sz * 4,
		acc.count, comp_sz * 4});
...
static void	fill_weights(t_json_value *json, char *bin, t_bone_weight *dst,
		int id)
{
	t_accessor acc;
	t_buffer_view bv;
	...
	while (++i < (int)acc.count)
	{
		...
		while (++k < 4)
		{
			if (acc.component_type == 5126)
				...
				dst[i].weights[k] = ((unsigned short *)tmp)[i * 4 + k]
					/ 65535.0;
			else
