...
/*   Updated: 2026/04/04 23:16:44 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	build_triangles(t_mesh *m)
{
	...
	n = count / 3;
	m->indices = malloc(sizeof(size_t) * count);
	if (!m->indices)
		return (false);
	ft_memcpy(m->indices, m->base_geometry.indices, sizeof(size_t) * count);
	m->tri_count = n;
	return (true);
}

static void	compute_bbox(t_mesh *m)
{
	size_t	i;
...
static void	apply_node_transform(t_mesh *m)
{
	...
	while (i < m->vertex_count)
	{
		/* Apply Node Transform in GLB Space */
		m->vertices[i].pos = mat4_mul_pos(m->node_transform, m->base_geometry.vertices[i]);
		if (m->base_normals)
			m->vertices[i].normal = mat4_mul_dir(m->node_transform, m->base_normals[i]);

		/* Y-Z Axis Alignment Swap (glTF Y-Up -> Engine Z-Up) */
...
void	glb_finalize_mesh(t_mesh *m)
{
	...
	/* Synchronize flat normal array for raytracer */
