...
static int	compare_x(const void *a, const void *b)
{
	...
	ia = (t_build_item *)a;
	ib = (t_build_item *)b;
	return (ia->centroid.x < ib->centroid.x ?
		-1 : ia->centroid.x > ib->centroid.x);
}
...
static int	compare_y(const void *a, const void *b)
{
	...
	ia = (t_build_item *)a;
	ib = (t_build_item *)b;
	return (ia->centroid.y < ib->centroid.y ?
		-1 : ia->centroid.y > ib->centroid.y);
}
...
static int	compare_z(const void *a, const void *b)
{
	...
	ia = (t_build_item *)a;
	ib = (t_build_item *)b;
	return (ia->centroid.z < ib->centroid.z ?
		-1 : ia->centroid.z > ib->centroid.z);
}
...
static t_bvh_node	*build_recursive(t_build_item *items, size_t count)
{
	...
	while (axis < 3)
	{
		...
		/* Better Algorithm: Binned SAH with 128 bins (high res)
			+ Termination Check */
		for (int s = 1; s < 32; s++)
		{
			split = (count * s) / 32;
			...
			for (size_t k = 0; k < split; k++)
				left_box = aabb_union(&left_box, &items[k].bbox);
			for (size_t k = split; k < count; k++)
				right_box = aabb_union(&right_box, &items[k].bbox);
			cost = t_cost + (aabb_surface_area(left_box) / p_area) * split
				* i_cost + (aabb_surface_area(right_box) / p_area) * (count
					- split) * i_cost;
			if (cost < min_cost)
			{
				min_cost = cost;
	...
	if (best_axis == 0)
		...
		qsort(items, count, sizeof(t_build_item), compare_y);
	/* If best_axis == 2,
		items are already sorted by Z from the last iteration */
