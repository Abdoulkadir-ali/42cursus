...
{
	t_gjk_shape sa = {cap, gjk_support_capsule, cap->phys.center};
	t_gjk_shape sb;
	int p = idx + 1;
	while (p < s->capsule_count && count < max)
		if (aabb_overlap(caabb, capsule_aabb(&s->capsules[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &cap->phys, &s->capsules[p].phys, &cap->transform, &s->capsules[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->cylinder_count && count < max)
		if (aabb_overlap(caabb, cylinder_aabb(&s->cylinders[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &cap->phys, &s->cylinders[p].phys, &cap->transform, &s->cylinders[p].transform, &c[count]);
		}
	p = -1;
	while (++p < s->rect_count && count < max)
		if (aabb_overlap(caabb, rect_aabb(&s->rects[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &cap->phys, &s->rects[p].phys, &cap->transform, &s->rects[p].transform, &c[count]);
		}
	p = -1;
	...
	while (++p < s->pyramid_count && count < max)
		if (aabb_overlap(caabb, pyramid_aabb(&s->pyramids[p])))
		{
			...
			count += gjk_make_contact(&sa, &sb, &cap->phys, &s->pyramids[p].phys, &cap->transform, &s->pyramids[p].transform, &c[count]);
		}
	return (count);
}
