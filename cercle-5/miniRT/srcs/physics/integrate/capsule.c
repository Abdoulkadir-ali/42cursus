...
void	integrate_capsule(t_capsule *cap, double dt)
{
	...
	cap->phys.velocity = vec3_add(cap->phys.velocity, vec3_scale(gravity_vec(),
				dt));
	cap->phys.velocity = vec3_scale(cap->phys.velocity, clamp_d(1.0
				- GLOBAL_DAMPING * dt, 0, 1));
	...
	cap->axis = vec3_norm(rot_by_ang(cap->axis, cap->phys.angular_velocity,
				dt));
