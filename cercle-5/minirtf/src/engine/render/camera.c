...
void	update_camera(t_app *app)
{
	...
	moved = 0;
	cam_changed = 0;
	   ui_dragging = (in->mouse_left && app->ui.active_id != 0);
	...
	if (key_held(in, XK_Shift_L))
		speed *= 4.0f;
	if (key_held(in, XK_w) || key_held(in, KEY_Z) || key_held(in, XK_z) || key_held(in, XK_W))
	{
	...
	if (key_held(in, XK_a) || key_held(in, XK_q) || key_held(in, XK_A) || key_held(in, XK_Q))
	{
