# 🧾 **Includes** - *Public Interfaces (Bonus)*

> **The contract layer of philosophers bonus**  
> A compact header layer that exposes the structures, constants, and function declarations shared by the simulation engine and bonus routines.
...
The `includes/` directory gathers the public header used across the whole philosophers bonus project. It defines the shared structures, compile-time flags, and function prototypes that connect argument parsing, simulation rules, bonus routines, and utility helpers through one stable interface.
...
| :---           | :--- |
| `philo_bonus.h`| Main project header exposing rules, philosopher structs, semaphores, routines, and utility interfaces |

---

## ⚙️ **How It Works**

Each implementation file under `srcs/` depends on declarations from `philo_bonus.h`. That header centralizes the project contract so modules can stay focused on implementation while still sharing the same types and function signatures.

It provides a common interface for:
- simulation rules and configuration
- philosopher and process/thread data
- semaphores and synchronization primitives
- argument parsing and validation helpers
- routines (eat, sleep, think, death)
...
```text
core <-> routine <-> lib
routine <-> process/thread data / semaphores
lib <-> printing / timing / conversion
```

`philo_bonus.h` acts as the shared vocabulary for the entire project.
