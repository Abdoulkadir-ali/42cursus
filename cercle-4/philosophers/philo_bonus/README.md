# 🧭 **Philo Bonus Module Pipeline**

> **A high-level map of the `philo_bonus/` tree.**  
> This README explains how the folders fit together in the global philosophers bonus pipeline, from argument parsing and setup to thread routines and simulation control.

![Language](https://img.shields.io/badge/Language-C-00599C?style=for-the-badge&logo=c&logoColor=white)
![Scope](https://img.shields.io/badge/Scope-philo_bonus%2F-overview-1f6feb?style=for-the-badge)
...
The `philo_bonus/` directory is the operational heart of the bonus project. It contains the runtime code that turns simulation parameters into a working philosopher pipeline with bonus features (such as process-based concurrency, semaphores, or additional requirements).
...
The full runtime pipeline across `philo_bonus/` is:
...
3. `srcs/routine/` sets up semaphores, philosopher data, processes/threads, and monitor.
4. Each philosopher process/thread runs its routine (eat, sleep, think, check death).
5. `srcs/routine/death.c` and monitor detect death and meal completion.
...
### `includes/`
- Project-wide types and function prototypes for bonus features.

### `srcs/core/`
- Starts the program and validates arguments.
- Owns simulation setup and launch.
- Bridges argument parsing and thread/process routine startup.

### `srcs/routine/`
- Implements thread/process routines for philosopher actions.
...
For a normal simulation, the trip through `philo_bonus/` looks like this:

1. `srcs/core/main.c` starts the simulation and parses arguments.
2. `srcs/core/parse.c` fills the rules struct.
3. `srcs/routine/routine.c` sets up processes/threads and launches the main loop.
4. Each philosopher process/thread runs its routine (eat, sleep, think, check death).
5. `srcs/routine/death.c` and monitor detect death and meal completion.
...
This README matches the structure and style of EXAMPLE.md, tailored for the philosophers bonus module.
