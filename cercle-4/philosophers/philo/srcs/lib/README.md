# Lib Module (Philosophers)

Provides utility functions for string conversion, printing, and timing. These helpers are used throughout the simulation for argument parsing, status output, and precise time management.
...
- `srcs/lib/` — Utility function root
  ...
  - `print.c` — Thread-safe status printing
...
### `print.c`
- Provides `print_status` for thread-safe status output.
- Uses `ft_putnbr` for printing numbers and `ft_putstr_fd` for messages.
- Locks the dead_flag mutex to ensure output consistency.
...
## 🛠️ Developer Notes
- All functions are designed for thread safety and minimal dependencies.
...
This README matches the structure and style of the core module, tailored for the philosophers project lib module.