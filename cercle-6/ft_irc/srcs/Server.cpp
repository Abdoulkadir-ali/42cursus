...
Server::Server(int port, const std::string &password)
	: _port(port), _password(password), _server_name("ircserv"),
	  ...
	  _bot(NULL), _dcc_relay(NULL), _bot_pipe_fd(-1)
{
	registerCommands();
}
	...
	while (!g_stop) {
		...
		for (size_t i = 0; i < _pfds.size(); ++i) {
			...
			try {
				...
				if (_bot && _pfds[i].fd == _bot_pipe_fd) {
					if (_pfds[i].revents & POLLIN) _bot->onRead();
					continue;
				}
				if (_dcc_relay && _dcc_relay->hasFd(_pfds[i].fd)) {
...
DccRelay			*Server::getDccRelay() const { return _dcc_relay; }
