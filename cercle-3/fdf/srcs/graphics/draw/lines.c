...
/*   Updated: 2025/11/22 05:07:27 by abdoali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	bresenham_fixed(t_graphics *g, t_bresenham_params p)
{
	...
	fp_r = (int)(p.r * (1 << FP_SHIFT));
	fp_g = (int)(p.green * (1 << FP_SHIFT));
	fp_b = (int)(p.b * (1 << FP_SHIFT));
	fp_dr = (int)(p.dr * (1 << FP_SHIFT));
	fp_dg = (int)(p.dg * (1 << FP_SHIFT));
	fp_db = (int)(p.db * (1 << FP_SHIFT));
	...
	while (1)
	{
		color = create_color(fp_r >> FP_SHIFT, fp_g >> FP_SHIFT,
				fp_b >> FP_SHIFT);
		pdp.pixel_addr = p.pixel_addr;
		pdp.z_addr = p.z_addr;
		pdp.zr = (float)p.zr;
		...
		if (e2 > -p.delta.y)
		{
			err -= p.delta.y;
			current.x += p.sign.x;
			if (current.x < 0 || current.x >= p.ctx.width)
				break ;
		...
		if (e2 < p.delta.x)
		{
			err += p.delta.x;
			current.y += p.sign.y;
			if (current.y < 0 || current.y >= p.ctx.height)
				break ;
