		...
		t_vec2 src_pos)
{
	...
	ctx.p2 = src->points.raw[ctx.idx + 1];
	ctx.c1 = src->points.color[ctx.idx];
	ctx.c2 = src->points.color[ctx.idx + 1];
	ctx.sc1 = src->points.source_color[ctx.idx];
	ctx.sc2 = src->points.source_color[ctx.idx + 1];
	set_ctx.dst_pos = dst_pos;
	set_ctx.pos = mix_pos(ctx.p1, ctx.p2, 0.5);
	set_ctx.color = interpolate_color(ctx.c1, ctx.c2, 0.5);
